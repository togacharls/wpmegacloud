...
//require_once 'mega.api.php';
/*require_once 'mega.crypto.php';
require_once 'mega.exception.php';
require_once 'mega.rsa.php';
require_once 'mega.util.php';*/

define("MEGA_FILE", 0);
define("MEGA_FOLDER", 1);
define("MEGA_ROOT_FOLDER", 2);
define("MEGA_INBOX", 3);
define("MEGA_TRASH_BIN", 4);
define("ERROR_NO_VALID_SESSION_TOKEN", "YTozOntzOjM6InVfayI7TjtzOjU6InVfc2lkIjtOO3M6NzoidV9wcml2ayI7Tjt9\r\n");
...
class MEGA {
  ...
  public function node_file_save($node, $dir_path = NULL, $filename = NULL, $args = array()) {
     ...
     */
    public function upload_file($filename, $parent_dir=NULL) { 

        $data = file_get_contents($filename);
        $size = strlen($data);

        //Primero se hace una petición a MEGA para obtener el enlace al que subir un fichero con tamaño "s"
        $req_url = array(array('a' => 'u', 's' => $size));
        $resp_url = $this->api_req($req_url);
        $ul_url = $resp_url[0]["p"];
    
        //Se generan las claves para el encriptado
        $ul_key = array(0, 1, 2, 3, 4, 5);
        for ($i = 0; $i < 6; $i++) {
            $ul_key[$i] = rand(0, 0xFFFFFFFF);
        }
    
        $key_ = array_slice($ul_key, 0, 4);
        $key_iv = array_slice($ul_key, 4, 2);
        $key_str = MEGAUtil::a32_to_str($key_);
        $key_str_iv = MEGAUtil::a32_to_str(array($ul_key[4], $ul_key[5], 0, 0));
    
        //Se encripta el fichero y se envía a la URL indicada
        $data_crypted = MEGACrypto::encrypt_aes_ctr($data, $key_str, $key_str_iv);
        $completion_handle = $this->http_do_request($ul_url, $data_crypted);
    
        $data_mac = MEGACrypto::cbc_mac($data, $key_str, $key_iv); 
        $meta_mac = array($data_mac[0] ^ $data_mac[1], $data_mac[2] ^ $data_mac[3]);
    
        //Se codifican los atributos del fichero
        $attributes = array('n' => basename($filename));
        
        $enc_attributes = MEGACrypto::enc_attr($attributes, $key_str);
        $key_final = array($ul_key[0] ^ $ul_key[4], $ul_key[1] ^ $ul_key[5], $ul_key[2] ^ $meta_mac[0], $ul_key[3] ^ $meta_mac[1], $ul_key[4], $ul_key[5], $meta_mac[0], $meta_mac[1]);
    
        $new_node = array(
            'h' => $completion_handle,
            't' => MEGA_FILE,
            'a' => MEGAUtil::base64urlencode($enc_attributes),
            'k' => MEGAUtil::a32_to_base64(MEGACrypto::encrypt_key($this->u_k, $key_final))
        );
        ...
        return $this->api_req($req);
    }
    
    /*
     * Create a new folder node
     * *Based on:  https://github.com/t3rm1n4l/go-mega/blob/master/mega.go#L1067
     * @param $foldername = STRING
     * @param $parent_node_id = STRING
     */
    public function create_folder($foldername, $parent_node_id=NULL){
        
        $attributes = array('n' => $foldername);
        
        //Se encripta el nodo de la misma forma que si fuese un fichero que se va a subir
        $ul_key = array(0, 1, 2, 3, 4, 5);
        for ($i = 0; $i < 6; $i++) {
            $ul_key[$i] = rand(0, 0xFFFFFFFF);
        }
        ...
        $data_mac = MEGACrypto::cbc_mac(NULL, $key_str, $key_iv);
        $meta_mac = array($data_mac[0] ^ $data_mac[1], $data_mac[2] ^ $data_mac[3]);
  ...
  protected function file_download_url($url, $size, $key, $dest) {
    ...
    $chunks = MEGAUtil::get_chunks($size);
...
class MEGACrypto {
    ...
    //AES encrypt in CTR mode
    public static function encrypt_aes_ctr($data, $key, $iv) {
        return mcrypt_encrypt(MCRYPT_RIJNDAEL_128, $key, $data, 'ctr', $iv);
    }

    public static function decrypt_aes_ctr($data, $key, $iv) {
    ...
    public static function encrypt_key($key, $a) {
        $x = array();
        $key_str = MEGAUtil::a32_to_str($key);
        for ($i = 0; $i < count($a); $i += 4) {
            $a_slice_str = MEGAUtil::a32_to_str(array_slice($a, $i, 4));
            $cbc = self::encrypt_aes_cbc($key_str, $a_slice_str);
            $cbc_str = MEGAUtil::str_to_a32($cbc);
            $x = array_merge($x, $cbc_str);
        }
        return $x;
    }
    ...
    public static function enc_attr($attr, $key) {
        ...
        return MEGACrypto::encrypt_aes_cbc($key, $attr);
    }
...
class MEGAUtil {
    ...
    public static function str_depad($b) {
        for ($i = strlen($b); $i-- && !self::uniord($b[$i]); );
        $b = substr($b, 0, $i + 1);
        return $b;
    }
...
class MEGA_NODE{
        ...
        foreach ($keys as $key){
            switch ($key){
                ...
                case "s":
                    ...
                    break;
                case "ts":
                    $this->lastModification = $node[$key];
                    break;
            }
        }
    }
    ...
    public function toArray(){
        ...
        if(isset($this->lastModification)){
            $array["ts"] = $this->lastModification;
        }
    ...
    function setLastModification($lastModification) {
...
/*
 * Class to use MEGA nodes which are files
 */
class MEGA_FILE{
    private $name;      //n
    private $c;         //c
...
/*
 * Class to use MEGA nodes which are links
 */
class MEGA_LINK{
    private $id;        //h
    private $url;       //ph
